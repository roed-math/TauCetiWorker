...
- **Operator target list.** If the assigned target below is not `none`, the worker has already chosen your target from the operator's list of milestones on the path to a specific goal, and already holds the claim `author/<target-roadmap>/<slug>` for it. The block after it is the rest of that area's list, for context: its `[ ]`/`[~]`/`[x]` statuses are LIVE (refreshed from open and merged PRs this round), so trust them over any copy of the file. You MUST author the assigned target, or a roadmap-local prerequisite it genuinely needs (state in the PR body why the target cannot land without it); you must NOT pick a different item on your own, however tempting. Use the assigned slug verbatim as the claim key and as the `id` in the target marker. If the open and merged PRs show the assigned target is already covered, or it turns out to be blocked, stop without a PR and report why — the worker reassigns next round. Prerequisite-following across roadmaps is allowed only when the prerequisite is itself a listed item or a roadmap-local prerequisite of the assigned one.
  __ASSIGNED__
  ```
  __TARGETS__
  ```
