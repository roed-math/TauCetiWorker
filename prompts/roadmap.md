...
- **Operator target list.** If `__TARGETS__` below is not `none`, it is the operator's ordered list of milestones on the path to a specific goal, restricted to the roadmap you were assigned. You MUST choose from it: take the first `[ ]` item whose `needs` are all `[x]` (or already merged on `main`) and that no open or merged PR already covers; if that item is claimed or in flight, take the next. Use its slug verbatim as the claim key `author/<target-roadmap>/<slug>` and as the `id` in the target marker. If no item qualifies, stop without a PR and say which items were blocked and why. Prerequisite-following across roadmaps is allowed only when the prerequisite is itself a listed item or a roadmap-local prerequisite of a listed item.
  ```
  __TARGETS__
  ```
